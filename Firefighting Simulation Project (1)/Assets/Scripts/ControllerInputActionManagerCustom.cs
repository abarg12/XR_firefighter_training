...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.StarterAssets
{
    ...
    public class ControllerInputActionManager : MonoBehaviour
    {
        ...
        [SerializeField]
        [Tooltip("If true, allows turn (snap or continuous) to remain active alongside smooth motion. Also strips any thumbstick binding from turn actions at runtime, so turning comes only from non-thumbstick bindings (e.g. A/B composite). Use for single-controller setups like CAVE.")]
        bool m_AllowMoveAndTurn;
        ...
        public bool allowMoveAndTurn
        {
            get => m_AllowMoveAndTurn;
            set
            {
                m_AllowMoveAndTurn = value;
                UpdateLocomotionActions();
            }
        }
        ...
        void UpdateLocomotionActions()
        {
            ...
            if (m_AllowMoveAndTurn)
            {
                // CAVE-style: allow move and turn to coexist on the same controller.
                SetEnabled(m_Turn, m_SmoothTurnEnabled);
                SetEnabled(m_SnapTurn, !m_SmoothTurnEnabled);

                // Strip thumbstick bindings from turn actions so the stick is move-only
                // and turning comes exclusively from non-thumbstick bindings (e.g. A/B composite).
                // Done once per component lifetime — overrides persist across enable/disable cycles.
                if (!m_ThumbstickStripped)
                {
                    StripThumbstickFromTurnActions();
                    m_ThumbstickStripped = true;
                }
            }
            else
            {
                // Default: disable turning when using continuous movement (original behavior).
        ...
        void StripThumbstickFromTurnActions()
        {
            DisableThumbstickBindings(m_Turn);
            DisableThumbstickBindings(m_SnapTurn);
        }

        static void DisableThumbstickBindings(InputActionReference actionReference)
        {
            ...
            for (int i = 0; i < action.bindings.Count; i++)
            {
                ...
                // Skip the composite parent entry — we only want to silence concrete path bindings.
                // Sub-bindings inside the A/B composite resolve to primaryButton/secondaryButton,
                // so they won't match the thumbstick filter below and will remain active.
                ...
                if (lowered.Contains("thumbstick") || lowered.Contains("primary2daxis"))
                {
                    // Override with empty path — binding stays in the asset but receives no input.
